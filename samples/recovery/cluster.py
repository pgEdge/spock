...
def _run_crash_scenario(pg_manager, spock_setup, config, formatter, port_start, processes, verbose, freeze_xids=False):
    ...
    try:
        # Step 1: Drop and create multiple test tables on all nodes
        test_tables = [
            {
                'name': 'crash_test',
                'schema': 'CREATE TABLE crash_test (id SERIAL PRIMARY KEY, data TEXT, created_at TIMESTAMP DEFAULT NOW());'
            },
            {
                'name': 'recovery_table_1',
                'schema': 'CREATE TABLE recovery_table_1 (id SERIAL PRIMARY KEY, name TEXT, value INTEGER, status TEXT);'
            },
            {
                'name': 'recovery_table_2',
                'schema': 'CREATE TABLE recovery_table_2 (id SERIAL PRIMARY KEY, category TEXT, amount NUMERIC(10,2), updated_at TIMESTAMP DEFAULT NOW());'
            },
            {
                'name': 'recovery_table_3',
                'schema': 'CREATE TABLE recovery_table_3 (id SERIAL PRIMARY KEY, user_id INTEGER, action TEXT, timestamp TIMESTAMP DEFAULT NOW());'
            }
        ]
        
        formatter.success(f"Creating {len(test_tables)} test tables on all nodes", port=None, indent=1)
        for port in [port_n1, port_n2, port_n3]:
            ...
            try:
                for table_info in test_tables:
                    ...
                    pg_manager.execute_sql(conn, f"DROP TABLE IF EXISTS {table_name} CASCADE;")
                    # Create table
                    pg_manager.execute_sql(conn, table_schema)
                    
                    # Verify table was created
                    verify_result = pg_manager.fetch_sql(conn, f"""
                        SELECT EXISTS (
                            SELECT 1 FROM information_schema.tables 
                    ...
                    if not verify_result or not verify_result[0][0]:
                        conn.close()
                        raise RuntimeError(f"Table {table_name} was not created on port {port}")
                    
                    # Add to replication set
                    try:
                        in_repset = pg_manager.fetch_sql(conn, f"""
                            SELECT EXISTS (
                        ...
                        if not (in_repset and in_repset[0][0]):
                            pg_manager.execute_sql(conn, f"SELECT spock.repset_add_table('default', '{table_name}');")
                    except Exception:
                        pass  # Table already in replication set or check failed, that's fine
                        
            except Exception as e:
                conn.close()
                raise RuntimeError(f"Failed to create test tables on port {port}: {e}")
            conn.close()
        
        time.sleep(1)
        
        # Step 2: Verify all tables exist on n1 before inserting
        conn_n1 = pg_manager.connect(port_n1)
        try:
            for table_info in test_tables:
                table_name = table_info['name']
                table_exists = pg_manager.fetch_sql(conn_n1, f"""
                    SELECT EXISTS (
                        SELECT 1 FROM information_schema.tables 
                        WHERE table_schema = 'public' AND table_name = '{table_name}'
                    );
                ...
                if not table_exists or not table_exists[0][0]:
                    conn_n1.close()
                    raise RuntimeError(f"Table {table_name} does not exist on n1 after creation")
                # Verify we can query it
                pg_manager.fetch_sql(conn_n1, f"SELECT COUNT(*) FROM {table_name};")
        except Exception as e:
            conn_n1.close()
            raise RuntimeError(f"Table verification failed on n1: {e}")
        conn_n1.close()
        ...
        # Step 4: Insert initial data into all tables (both n2 and n3 receive)
        formatter.success("Inserting initial data into all tables on n1 (both n2 and n3 receive)", port=None, indent=1)
        conn_n1 = pg_manager.connect(port_n1)
        ...
        # recovery_table_1: 15 rows
        for i in range(15):
            pg_manager.execute_sql(conn_n1, f"INSERT INTO recovery_table_1 (name, value, status) VALUES ('item_{i+1}', {i+1}, 'active');")
        
        # recovery_table_2: 10 rows
        for i in range(10):
            pg_manager.execute_sql(conn_n1, f"INSERT INTO recovery_table_2 (category, amount) VALUES ('cat_{i+1}', {(i+1)*10.5});")
        
        # recovery_table_3: 12 rows
        for i in range(12):
            pg_manager.execute_sql(conn_n1, f"INSERT INTO recovery_table_3 (user_id, action) VALUES ({i+1}, 'action_{i+1}');")
        ...
        for attempt in range(max_wait):
            ...
            n2_t1 = pg_manager.fetch_sql(conn_n2, "SELECT count(*) FROM recovery_table_1;")[0][0]
            n2_t2 = pg_manager.fetch_sql(conn_n2, "SELECT count(*) FROM recovery_table_2;")[0][0]
            n2_t3 = pg_manager.fetch_sql(conn_n2, "SELECT count(*) FROM recovery_table_3;")[0][0]
            conn_n2.close()
            
            conn_n3 = pg_manager.connect(port_n3)
            n3_crash = pg_manager.fetch_sql(conn_n3, "SELECT count(*) FROM crash_test;")[0][0]
            n3_t1 = pg_manager.fetch_sql(conn_n3, "SELECT count(*) FROM recovery_table_1;")[0][0]
            n3_t2 = pg_manager.fetch_sql(conn_n3, "SELECT count(*) FROM recovery_table_2;")[0][0]
            n3_t3 = pg_manager.fetch_sql(conn_n3, "SELECT count(*) FROM recovery_table_3;")[0][0]
            conn_n3.close()
            
            if (n2_crash == 20 and n3_crash == 20 and 
                n2_t1 == 15 and n3_t1 == 15 and
                n2_t2 == 10 and n3_t2 == 10 and
                n2_t3 == 12 and n3_t3 == 12):
                formatter.success(f"Initial sync complete: n2=(crash:{n2_crash}, t1:{n2_t1}, t2:{n2_t2}, t3:{n2_t3}), n3=(crash:{n3_crash}, t1:{n3_t1}, t2:{n3_t2}, t3:{n3_t3})", port=None, indent=1)
                break
            
            if verbose and attempt % 5 == 0:
                formatter.info(f"Waiting for replication... (attempt {attempt+1}/{max_wait})", port=None, indent=2)
        else:
            # Timeout - check what we have
            raise RuntimeError(f"Replication timeout after {max_wait}s")
        ...
        if not freeze_xids:
            ...
            conn_n2 = pg_manager.connect(port_n2)
            n2_before_crash = pg_manager.fetch_sql(conn_n2, "SELECT count(*) FROM crash_test;")[0][0]
            n2_before_t1 = pg_manager.fetch_sql(conn_n2, "SELECT count(*) FROM recovery_table_1;")[0][0]
            n2_before_t2 = pg_manager.fetch_sql(conn_n2, "SELECT count(*) FROM recovery_table_2;")[0][0]
            n2_before_t3 = pg_manager.fetch_sql(conn_n2, "SELECT count(*) FROM recovery_table_3;")[0][0]
            conn_n2.close()
            time.sleep(3)  # Wait a bit more
            conn_n2 = pg_manager.connect(port_n2)
            n2_after_crash = pg_manager.fetch_sql(conn_n2, "SELECT count(*) FROM crash_test;")[0][0]
            n2_after_t1 = pg_manager.fetch_sql(conn_n2, "SELECT count(*) FROM recovery_table_1;")[0][0]
            n2_after_t2 = pg_manager.fetch_sql(conn_n2, "SELECT count(*) FROM recovery_table_2;")[0][0]
            n2_after_t3 = pg_manager.fetch_sql(conn_n2, "SELECT count(*) FROM recovery_table_3;")[0][0]
            conn_n2.close()
            
            if (n2_before_crash != n2_after_crash or n2_before_t1 != n2_after_t1 or 
                n2_before_t2 != n2_after_t2 or n2_before_t3 != n2_after_t3):
                raise RuntimeError(f"n2 is still receiving data after suspension!")
            
            formatter.success(f"Verified n2 stopped receiving data from n1 (all tables stable)", port=None, indent=1)
        
        # Step 7: Insert additional rows into all tables (only n3 receives, n2's subscription from n1 is suspended)
        if not freeze_xids:
            formatter.success("Inserting additional rows into all tables on n1 (only n3 receives, n2's subscription from n1 is suspended)", port=None, indent=1)
        else:
            formatter.success("Inserting additional rows into all tables on n1", port=None, indent=1)
        conn_n1 = pg_manager.connect(port_n1)
        
        # crash_test: 70 more rows (total will be 90 on n3, 20 on n2)
        for i in range(70):
            pg_manager.execute_sql(conn_n1, f"INSERT INTO crash_test (data) VALUES ('lag_{i+21}');")
        
        # recovery_table_1: 25 more rows (total will be 40 on n3, 15 on n2)
        for i in range(25):
            pg_manager.execute_sql(conn_n1, f"INSERT INTO recovery_table_1 (name, value, status) VALUES ('item_{i+16}', {i+16}, 'pending');")
        
        # recovery_table_2: 20 more rows (total will be 30 on n3, 10 on n2)
        for i in range(20):
            pg_manager.execute_sql(conn_n1, f"INSERT INTO recovery_table_2 (category, amount) VALUES ('cat_{i+11}', {(i+11)*10.5});")
        
        # recovery_table_3: 18 more rows (total will be 30 on n3, 12 on n2)
        for i in range(18):
            pg_manager.execute_sql(conn_n1, f"INSERT INTO recovery_table_3 (user_id, action) VALUES ({i+13}, 'action_{i+13}');")
        ...
        if n3_crash <= n2_crash or n3_t1 <= n2_t1 or n3_t2 <= n2_t2 or n3_t3 <= n2_t3:
            raise RuntimeError(f"n3 is not ahead! n2=(crash:{n2_crash}, t1:{n2_t1}, t2:{n2_t2}, t3:{n2_t3}), n3=(crash:{n3_crash}, t1:{n3_t1}, t2:{n3_t2}, t3:{n3_t3})")
        
        formatter.success(
            f"Pre-crash state: n2=(crash:{n2_crash}, t1:{n2_t1}, t2:{n2_t2}, t3:{n2_t3}), n3=(crash:{n3_crash}, t1:{n3_t1}, t2:{n3_t2}, t3:{n3_t3})",
            port=None, indent=1
        )
        ...
        formatter.success(f"  crash_test: {n2_crash_final} rows", port=None, indent=1)
        formatter.success(f"  recovery_table_1: {n2_t1_final} rows", port=None, indent=1)
        formatter.success(f"  recovery_table_2: {n2_t2_final} rows", port=None, indent=1)
        formatter.success(f"  recovery_table_3: {n2_t3_final} rows", port=None, indent=1)
        ...
        formatter.success(f"  crash_test: {n3_crash_final} rows", port=None, indent=1)
        formatter.success(f"  recovery_table_1: {n3_t1_final} rows", port=None, indent=1)
        formatter.success(f"  recovery_table_2: {n3_t2_final} rows", port=None, indent=1)
        formatter.success(f"  recovery_table_3: {n3_t3_final} rows", port=None, indent=1)
        ...
        formatter.success(f"  n3 (ahead) - SOURCE for recovery:", port=None, indent=1)
        formatter.success(f"    crash_test: {n3_crash_final} rows", port=None, indent=2)
        ...
        # Print RECOVERY COMMANDS at the very end (no timestamp, no elapsed time)
        print()
        print("=" * 72)
        print("RECOVERY COMMANDS - Run these on n2 (target node):")
        print("=" * 72)
        print()
        print("1. Comprehensive Recovery (recover ALL missing data from n3):")
        ...
        print(f"   );\"")
        print()
        print("2. Origin-Aware Recovery (recover ONLY n1-origin transactions):")
        print(f"   psql -p {port_n2} {config.DB_NAME} -c \"")
        print(f"   CALL spock.recover_cluster(")
        print(f"       p_source_dsn := 'host=localhost port={port_n3} dbname={config.DB_NAME} user={config.DB_USER}',")
