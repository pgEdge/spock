    ...
    try:
        ...
        for port in [port_n1, port_n2, port_n3]:
            ...
            try:
                ...
                pass  # Table already in replication set, that's fine
            conn.close()
        ...
        for attempt in range(10):
            ...
            for port, node_name in [(port_n2, 'n2'), (port_n3, 'n3')]:
                ...
                try:
                    sub_result = pg_manager.fetch_sql(conn, f"""
                        SELECT s.sub_enabled, s.sub_rescue_suspended
                        FROM spock.subscription s
                        JOIN spock.node o ON s.sub_origin = o.node_id
                        WHERE o.node_name = 'n1' AND s.sub_target = (SELECT node_id FROM spock.node WHERE node_name = '{node_name}');
                    """)
                    if sub_result and sub_result[0]:
                        sub_enabled, sub_rescue_suspended = sub_result[0]
                        if not sub_enabled or sub_rescue_suspended:
                            all_ready = False
                except Exception:
                    all_ready = False
                conn.close()
        ...
        formatter.success("Inserting 20 initial rows on n1 (both n2 and n3 receive)", port=port_n1, indent=1)
        conn_n1 = pg_manager.connect(port_n1)
        for i in range(20):
        ...
        # Step 6: Suspend ALL subscriptions TO n2 (from n1 and n3)
        conn_n2 = pg_manager.connect(port_n2)
        ...
        for sub_row in sub_n2_result:
            ...
            pg_manager.execute_sql(conn_n2, 
                f"UPDATE spock.subscription SET sub_rescue_suspended = true, sub_enabled = false WHERE sub_id = {sub_id};")
            formatter.success(f"Suspended n2's subscription '{sub_name}' from {origin_name}", port=port_n2, indent=1)
            if origin_name == 'n1':
                sub_n1_n2_id = sub_id
        ...
        # Verify n2 is not receiving more data (apply workers have stopped)
        conn_n2 = pg_manager.connect(port_n2)
        ...
        formatter.success(f"Verified n2 stopped receiving data ({n2_before_lag} rows stable)", port=port_n2, indent=1)
        
        # Step 7: Insert 70 more rows (only n3 receives, n2 is suspended)
        formatter.success("Inserting 70 more rows on n1 (only n3 receives, n2 suspended)", port=port_n1, indent=1)
        conn_n1 = pg_manager.connect(port_n1)
        for i in range(70):
        ...
        # Step 9: Re-enable n2's subscription from n1 (so recovery.sql can suspend it properly)
        # Keep n2's subscription from n3 suspended to prevent catch-up
        if sub_n1_n2_id:
            conn_n2 = pg_manager.connect(port_n2)
            pg_manager.execute_sql(conn_n2, 
                f"UPDATE spock.subscription SET sub_rescue_suspended = false, sub_enabled = true WHERE sub_id = {sub_n1_n2_id};")
            conn_n2.close()
            formatter.success(f"Re-enabled n2's subscription from n1 for recovery", port=port_n2, indent=1)
            time.sleep(2)  # Brief wait for subscription to be active
        
        # Step 10: Verify n2 and n3 are healthy (can connect, queries work)
        conn_n2 = pg_manager.connect(port_n2)
        ...
        # Step 11: Crash n1
        formatter.success("Crashing n1...", port=port_n1, indent=1)
        ...
        if n1_process:
            ...
            try:
                ...
                formatter.warning(f"Could not kill n1: {e}", port=port_n1, indent=1)
        ...
        if freeze_xids:
            ...
            # Suspend all subscriptions on n2
            try:
                ...
                pg_manager.execute_sql(conn_n2_freeze, """
                    ...
                    FROM spock.subscription WHERE sub_enabled = true;
                """)
                conn_n2_freeze.close()
                formatter.success("Suspended all subscriptions on n2 to freeze XIDs", port=port_n2, indent=2)
            except Exception as e:
                formatter.warning(f"Could not suspend subscriptions on n2: {e}", port=port_n2, indent=2)
            
            # Suspend all subscriptions on n3
            try:
                ...
                formatter.success("Suspended all subscriptions on n3 to freeze XIDs", port=port_n3, indent=2)
            except Exception as e:
                formatter.warning(f"Could not suspend subscriptions on n3: {e}", port=port_n3, indent=2)
        ...
        n2_subs = pg_manager.fetch_sql(conn_n2, """
            SELECT s.sub_name, o.node_name as origin, s.sub_enabled, s.sub_rescue_suspended
            FROM spock.subscription s
            JOIN spock.node o ON s.sub_origin = o.node_id
        ...
        formatter.success("=" * 70, port=None, indent=0)
        formatter.success("CRASH SCENARIO COMPLETE - FINAL STATE", port=None, indent=0)
        formatter.success("=" * 70, port=None, indent=0)
        
        formatter.success(f"\nNODE n2 (TARGET for recovery):", port=port_n2, indent=0)
        formatter.success(f"  • Row count: {n2_final} rows", port=port_n2, indent=1)
        formatter.success(f"  • LSN (n1->n2): {n2_lsn_final}", port=port_n2, indent=1)
        formatter.success(f"  • Subscriptions:", port=port_n2, indent=1)
        for sub_row in n2_subs:
            sub_name, origin, enabled, suspended = sub_row
            status = "ENABLED" if enabled and not suspended else "DISABLED/SUSPENDED"
            formatter.success(f"    - {sub_name} (from {origin}): {status}", port=port_n2, indent=2)
        
        formatter.success(f"\nNODE n3 (SOURCE for recovery):", port=port_n3, indent=0)
        formatter.success(f"  • Row count: {n3_final} rows", port=port_n3, indent=1)
        formatter.success(f"  • LSN (n1->n3): {n3_lsn_final}", port=port_n3, indent=1)
        formatter.success(f"  • Subscriptions:", port=port_n3, indent=1)
        for sub_row in n3_subs:
            sub_name, origin, enabled, suspended = sub_row
            status = "ENABLED" if enabled and not suspended else "DISABLED/SUSPENDED"
            formatter.success(f"    - {sub_name} (from {origin}): {status}", port=port_n3, indent=2)
        
        formatter.success(f"\nRECOVERY SCENARIO:", port=None, indent=0)
        ...
        formatter.success(f"\nRECOVERY COMMAND:", port=None, indent=0)
        formatter.success(
            f"  CALL spock.recover_run(",
            port=None, indent=1
        )
        formatter.success(
            f"    failed_node_name := 'n1',",
            port=None, indent=2
        )
        formatter.success(
            f"    source_node_name := 'n3',",
            port=None, indent=2
        )
        formatter.success(
            f"    source_dsn := 'host=localhost port={port_n3} dbname=pgedge user=pge password=1safepassword',",
            port=None, indent=2
        )
        formatter.success(
            f"    target_node_name := 'n2',",
            port=None, indent=2
        )
        formatter.success(
            f"    target_dsn := 'host=localhost port={port_n2} dbname=pgedge user=pge password=1safepassword'",
            port=None, indent=2
        )
        formatter.success(
            f"  );",
