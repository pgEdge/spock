 ...
 *	  the only consult is spock.quorum_status(), which invalidates first.
 *	  A consuming worker (group-slot eviction) is not built yet.
 *	- Must respect spock.quorum_timeout.  Overrunning it is UNKNOWN, not a
 *	  reason to keep waiting.  etcd applies it as an HTTP deadline; the
 *	  in-database providers apply it as SET LOCAL statement_timeout.
