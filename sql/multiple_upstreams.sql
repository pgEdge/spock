...
SELECT * FROM spock.create_node(node_name := 'test_provider1', dsn := (SELECT provider1_dsn FROM spock_regress_variables()) || ' user=super');
...
SELECT * FROM spock.replication_set_add_table('default', 'multi_ups_tbl', true);
...
SELECT * FROM spock.replication_set_add_table('default', 'multi_ups_tbl');
...
SELECT spock.wait_for_subscription_sync_complete('test_subscription1');
COMMIT;
...
\c :provider1_dsn
SELECT * FROM spock.drop_node(node_name := 'test_provider1');
\set VERBOSITY terse
DROP TABLE public.multi_ups_tbl CASCADE;

\c :subscriber_dsn
SELECT * FROM spock.drop_subscription('test_subscription1');
