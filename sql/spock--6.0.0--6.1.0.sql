...
--
-- First-time create (no prior physical slot) is always allowed: nothing was
-- pinning WAL yet. Recreating a slot that previously existed restarts
-- protection at the current WAL position, so it is refused in strict mode;
-- use spock.repair_group_slot('recreate') after switching to a non-strict
-- safety mode.
...
BEGIN
    ...
    SELECT last_advanced_lsn, repair_required, blocked_reason
      INTO v_last, v_repair, v_blocked
      FROM spock.group_slot_state
     WHERE node_id = v_local;

    -- A prior physical slot leaves a trace: we advanced, we already marked
    -- repair, or the worker recorded missing_slot_state. First-time init
    -- seeds a row with last_advanced_lsn = 0/0 and no blocked reason.
    v_ever := FOUND AND (COALESCE(v_repair, false)
                         OR v_blocked = 'missing_slot_state'
                         OR COALESCE(v_last, '0/0'::pg_lsn) <> '0/0'::pg_lsn);

    IF v_ever AND v_mode = 'strict' THEN
        RETURN true;
    END IF;
...
-- Resolve a member node id from its name.
--
-- spock.node is authoritative while the node still exists, but a departed
-- node is dropped from it before an operator reaches the group slot: the
-- catastrophic-failure runbook calls spock.node_drop() during cleanup, long
-- before anyone thinks about WAL retention.  Membership rows keep the name,
-- so fall back to them.  Without this fallback the part functions silently
-- do nothing once the node is gone, which is exactly when they are needed.
CREATE FUNCTION spock.group_slot_member_id(p_gen bigint, p_name name)
RETURNS oid
LANGUAGE sql STABLE AS $$
    SELECT COALESCE(
             (SELECT n.node_id FROM spock.node n WHERE n.node_name = p_name),
             (SELECT m.member_node_id
                FROM spock.group_slot_membership m
               WHERE m.membership_generation = p_gen
                 AND m.member_node_name = p_name
               LIMIT 1));
$$;
...
BEGIN
    ...
    -- A member that no longer exists in spock.node has been dropped from the
    -- cluster, so release its claim on the horizon.  Otherwise its progress
    -- row stays empty forever, evaluate() reports stale_progress, and the
    -- group slot pins WAL until the disk fills.  That is the catastrophic
    -- failure path: the node is gone, node_drop() runs during cleanup, and
    -- there is never an orderly part to clear the membership.
    --
    -- Restricted to 'active' members on purpose.  A 'joining' member is not
    -- in spock.node yet on every node, and a 'parting' one is mid-teardown;
    -- retiring either here would collapse the very freeze that protects it.
    UPDATE spock.group_slot_membership m
       SET state = 'removed', required = false, updated_at = now()
     WHERE m.membership_generation = p_gen
       AND m.state = 'active'
       AND NOT EXISTS (SELECT 1 FROM spock.node n
                        WHERE n.node_id = m.member_node_id);
...
BEGIN
    ...
    -- Remember a physical slot was observed so a later disappearance is
    -- treated as damage, not first-time create.
    SELECT confirmed_flush_lsn INTO v_cur
      FROM pg_replication_slots WHERE slot_name = v_slot;
    IF v_cur IS NOT NULL THEN
        UPDATE spock.group_slot_state
           SET last_advanced_lsn = v_cur
         WHERE node_id = v_local
           AND last_advanced_lsn = '0/0'::pg_lsn;
    END IF;

    IF v_reason IS NULL THEN
        IF v_safe IS NOT NULL THEN
            -- Compare against confirmed_flush_lsn, not restart_lsn: for a
            -- logical slot that is the minimum pg_replication_slot_advance()
            -- accepts, and restart_lsn trails it.  Guarding on restart_lsn
            -- would call advance with a target it rejects outright, failing
            -- the tick instead of simply having nothing to do.
            SELECT confirmed_flush_lsn INTO v_cur
              FROM pg_replication_slots WHERE slot_name = v_slot;
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_replication_slots WHERE slot_name = v_slot) THEN
        RAISE EXCEPTION 'group slot advancement blocked: missing_slot_state'
            USING ERRCODE = 'object_not_in_prerequisite_state',
                  HINT = 'Use spock.repair_group_slot() to recreate the physical slot.';
    END IF;
    ...
    -- confirmed_flush_lsn, not restart_lsn: see group_slot_worker_tick().
    SELECT confirmed_flush_lsn INTO v_cur
      FROM pg_replication_slots WHERE slot_name = v_slot;
    ...
    IF mode = 'recreate' AND NOT v_present THEN
        IF v_safety = 'strict' THEN
            RAISE EXCEPTION 'refusing to recreate a missing group slot in strict safety mode'
                USING ERRCODE = 'object_not_in_prerequisite_state',
                      HINT = 'Set spock.group_slots_safety_mode to a non-strict value (repair) to allow recreation (older WAL protection will be lost).';
        END IF;
...
BEGIN
    ...
    -- Resolved after v_gen: the fallback searches the current generation.
    v_part := spock.group_slot_member_id(v_gen, parting_node_name);
...
BEGIN
    ...
    IF v_part IS NULL THEN
        RAISE NOTICE 'node % is not a member of group-slot generation %; '
                     'clearing local part state only',
                     parting_node_name, v_gen;
    END IF;

    -- Carry remaining members forward.  The parting node is excluded, and so
    -- is any member already dropped from spock.node: carrying either one over
    -- would recreate the stale_progress block in the new generation.
    DELETE FROM spock.group_slot_membership WHERE membership_generation = v_newgen;

    INSERT INTO spock.group_slot_membership
            (membership_generation, member_node_id, member_node_name, state, required)
    SELECT v_newgen, m.member_node_id, m.member_node_name, 'active', true
      FROM spock.group_slot_membership m
     WHERE m.membership_generation = v_gen
       AND (v_part IS NULL OR m.member_node_id <> v_part)
       AND m.state <> 'removed'
       AND EXISTS (SELECT 1 FROM spock.node n
                    WHERE n.node_id = m.member_node_id);
