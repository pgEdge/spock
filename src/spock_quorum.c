...
spock_quorum_startup(void)
{
	...
	/*
	 * Every provider but 'none' addresses a shared key space, and the prefix
	 * is the only thing separating one cluster's members from another's.
	 * Refuse rather than fall back to a default: a wrong answer here mixes
	 * two clusters' membership, which is far worse than having no answer.
	 */
	if (spock_quorum_provider != SPOCK_QUORUM_PROVIDER_NONE &&
		(spock_quorum_cluster_id == NULL || spock_quorum_cluster_id[0] == '\0'))
	{
		note_error("spock.quorum_cluster_id is not set");
		ereport(WARNING,
				(errmsg("spock quorum: provider \"%s\" needs spock.quorum_cluster_id",
						active->name),
				 errhint("Set it to a value unique to this cluster. There is no "
						 "default, because two clusters sharing one would count "
						 "each other's nodes as their own.")));
		return;
	}
...
spock_quorum_refresh(void)
{
	...
	if (!active->refresh(&detail))
	{
		 ...
		 * would overwrite the reason the refresh failed.  Leave the snapshot
		 * invalid: every question this tick then answers conservatively.
		 */
...
snapshot_take(void)
{
	...
	if (snap_quorum == SPOCK_QUORUM_YES)
	{
		...
		if (snap_leader == SPOCK_QUORUM_UNKNOWN && detail != NULL)
			note_error(detail);

		/*
		 * The leader's name belongs to this reading too.  Fetching it later,
		 * when the status view is rendered, would be a separate consult and
		 * could name a leader from a different moment than the quorum answer
		 * shown beside it.
		 */
		if (active->leader != NULL)
		{
			char	   *who;

			detail = NULL;
			who = active->leader(&detail);
			if (who != NULL)
			{
				MemoryContext old = MemoryContextSwitchTo(snap_ctx);

				snap_leader_name = pstrdup(who);
				MemoryContextSwitchTo(old);
			}
		}
