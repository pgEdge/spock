...
spock_group_slot_init_local(Oid node_id)
{
	...
	/*
	 * Version-skew guard: with the 6.1.0 binary loaded but the extension still
	...
	{
		/*
		 * The probe itself failed.  Skip the seed rather than pressing on:
		 * the whole point of asking is to avoid a missing-relation error
		 * inside spock.create_node(), so an unanswered question has to be
		 * treated as "not present".
		 */
		elog(WARNING, "spock group slot: could not probe for group_slot_state "
			 "(rc=%d); skipping metadata seed", rc);
		SPI_finish();
		return;
	}
	else
...
/*
 * Does the installed extension provide the group-slot SQL yet?
 *
 * A 6.1.0 binary can be loaded while the extension is still at 6.0.0, in
 * which case spock.group_slot_worker_tick() does not exist.  Probing beats
 * letting every tick fail: the worker would otherwise emit a WARNING on each
 * cycle for as long as the operator takes to run ALTER EXTENSION.  Must be
 * called with an SPI connection and a snapshot already established.
 */
static bool
group_slot_sql_present(void)
{
	bool		isnull;
	Datum		present;

	if (SPI_execute("SELECT to_regprocedure('spock.group_slot_worker_tick()')"
					" IS NOT NULL", true, 1) != SPI_OK_SELECT ||
		SPI_processed != 1)
		return false;

	present = SPI_getbinval(SPI_tuptable->vals[0], SPI_tuptable->tupdesc,
							1, &isnull);
	return !isnull && DatumGetBool(present);
}
...
spock_group_slot_do_tick(void)
{
	...
	/* Latched while the extension is behind, so the notice is logged once. */
	static bool	skew_reported = false;
	...
	PG_TRY();
	{
		...
		if (!group_slot_sql_present())
		{
			if (!skew_reported)
			{
				ereport(LOG,
						(errmsg("spock group slot: the installed extension does "
								"not provide group-slot support yet"),
						 errhint("Run \"ALTER EXTENSION spock UPDATE\"; the "
								 "worker resumes on its own once it is done.")));
				skew_reported = true;
			}
		}
		else
