...
parse_json(const char *json)
{
	...
	/*
	 * An internal subtransaction, not a bare PG_TRY.  Catching the error with
	 * FlushErrorState() alone would leave the surrounding transaction
	 * aborted, and this runs inside whatever transaction the operator called
	 * spock.quorum_status() from -- so a single malformed reply would break
	 * their session rather than just this reading.
	 */
...
/*
 * Quorum by linearizable read.
 *
 * /v3/maintenance/status is deliberately NOT used: the Status RPC is answered
 * from the queried member's own state, so a member isolated in a minority
 * partition happily reports the leader it last knew about.  Treating that as
 * proof of quorum is exactly the mistake this layer exists to avoid.
 *
 * A Range read, by contrast, is linearizable unless serializable is asked for,
 * which means etcd only answers it from within a majority.  A successful reply
 * therefore is the proof.  Reading our own nodes prefix with limit 1 keeps the
 * response small and needs no extra key.
 *
 * A minority member cannot complete the read, so the call runs out its
...
etcd_have_quorum(char **errdetail)
{
	...
	char	   *body = psprintf("{\"key\":\"%s\",\"range_end\":\"%s\","
								"\"limit\":\"1\",\"serializable\":false}",
								b64(prefix), b64(prefix_end(prefix)));

	if (etcd_post("/v3/kv/range", body, errdetail) == NULL)
...
etcd_is_leader(char **errdetail)
{
	...
	if (etcd_lease_id == 0)
	{
		...
		/*
		 * startup() leaves this unset when there is no local node, and a
		 * comparison against it would dereference NULL.  Without an identity
