 ...
 *
 * A failed command is reported with ereport(ERROR), which does not return
 * to the caller, so every caller can treat the returned PGresult as valid
 * and is responsible for PQclear()ing it.
 */
static PGresult *
