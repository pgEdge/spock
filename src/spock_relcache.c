...
spock_relation_open(uint32 remoteid, LOCKMODE lockmode)
{
	...
	if (!OidIsValid(entry->reloid))
	{
		...
		for (i = 0; i < entry->natts; i++)
		{
			...
			/*
			 * A missing local column is handled like a missing local relation.
			 * Collect every missing column rather than stopping at the first,
			 * so an operator repairing a drifted schema gets the whole list at
			 * once instead of one column per apply attempt.  Skip the rest of
			 * the loop body: it indexes the local tupdesc by attmap[i].
			 */
								 ...
								 Oid *attrtypes, Oid *attrtypmods)
{
	...
	/* Local-schema validation requires the lock taken by spock_relation_open(). */
