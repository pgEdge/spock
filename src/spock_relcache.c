...
spock_relation_open(uint32 remoteid, LOCKMODE lockmode)
{
	...
	if (!OidIsValid(entry->reloid))
	{
		...
		for (i = 0; i < entry->natts; i++)
		{
			 ...
			 * Report it on the first pass so apply_work() records the cause,
			 * then return NULL during exception replay so the caller can apply
			 * the configured exception behavior instead of erroring again.
			 */
