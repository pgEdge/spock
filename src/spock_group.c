...
spock_checkpoint_hook(XLogRecPtr checkPointRedo, int flags)
{
	 ...
	 *
	 * resource.dat and WAL cooperate for crash recovery: - resource.dat seeds
	 * shmem with checkpoint-time values on restart - WAL replay
	 * (spock_rmgr_redo) advances any entries written after checkPointRedo
	 *
	 ...
	 * workers were idle during the checkpoint interval (provider
	 * unreachable), no SPOCK_RMGR_APPLY_PROGRESS records exist after
	 * checkPointRedo to recover from either, causing spock.progress to show
	 * stale values or missing entries after crash recovery.
	 */
	spock_group_resource_dump();
}
...
spock_group_progress_force_set_list(List *lst)
{
	...
	foreach(lc, lst)
	{
		...
		else if (entry->progress.remote_commit_lsn >= sap->remote_commit_lsn)
		{
			 ...
			 * value from read_peer_progress is authoritative because it was
			 * captured at COPY snapshot time.  The apply worker may have
			 * advanced past it since then, but any data it applied after the
			 * snapshot is NOT in the COPY — so the new node must replay
			 * from the snapshot boundary, not from the worker's current
			 * position.
			 */
		...
		{
			/*
			 * Stale entry below resume_lsn; will be reset after WAL write
			 * succeeds.
			 */
		}

		/*
		 * WAL-log before shmem update; skipped above when existing LSN is
		 * higher.
		 */
		spock_apply_progress_add_to_wal(sap);
