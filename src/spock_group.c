...
spock_group_shmem_request(void)
{
    ...
    #if 0
    Size nelem = (nelem_hint > 0 ? nelem_hint : SPOCK_GROUP_DEFAULT_NELEM);
    int  parts = (partitions_hint > 0 ? partitions_hint : SPOCK_GROUP_DEFAULT_PARTS);

    /* estimate shmem; dynahash will allocate more as needed */
    Size est = hash_estimate_size(nelem, sizeof(SpockGroupEntry));

    RequestAddinShmemSpace(est);
    RequestNamedLWLockTranche(SPOCK_GROUP_TRANCHE_NAME, parts);
    #endif
}
