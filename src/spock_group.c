...
static void write_one(int fd, const void *buf, size_t nbytes, const char *what);
static void read_one(int fd, void *buf, size_t nbytes, const char *what);
...
dump_one_group_cb(const SpockGroupEntry *e, void *arg)
{
    ...
    write_one(ctx->fd, &e->progress, sizeof(e->progress), "resource.dat(data)");
    ctx->count++;
}
...
spock_group_resource_dump(void)
{
    ...
    write_one(fd, &hdr, sizeof(hdr), "resource.dat(header)");
...
spock_group_resource_load(void)
{
    ...
    read_one(fd, &hdr, sizeof(hdr), "resource.dat(header)");
    ...
    for (uint32 i = 0; i < hdr.entry_count; i++)
    {
        ...
        read_one(fd, &sap, sizeof(sap), "resource.dat(data)");
        /*
...
static void
write_one(int fd, const void *buf, size_t nbytes, const char *what)
{
...
static void
read_one(int fd, void *buf, size_t nbytes, const char *what)
{
	int r;
	...
	if (buf == NULL || nbytes == 0)
		ereport(ERROR,
				(errmsg("invalid buffer or size for %s", what)));

	if ((r = read(fd, buf, nbytes)) != nbytes)
	{
		if (r < 0)
			ereport(ERROR,
					(errcode_for_file_access(),
					 errmsg("could not read file \"%s\": %m", SPOCK_RES_DUMPFILE)));
		else
			ereport(ERROR,
					(errcode(ERRCODE_DATA_CORRUPTED),
					 errmsg("could not read file \"%s\": read %d of %zu",
							SPOCK_RES_DUMPFILE, r, nbytes)));
