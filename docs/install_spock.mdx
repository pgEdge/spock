...
The latest Spock extension is automatically installed and created with any [pgEdge Distributed or Enterprise Postgres](../platform.mdx) installation. A deployment provides quick and easy access to: 

*  the latest minor version of your preferred Postgres version.
*  Spock [functions and procedures](spock_functions.mdx).
*  [ACE (the Active Consistency Engine) consistency monitoring](../platform/ace.mdx). 
*  [Snowflake sequences](../platform/snowflake.mdx).
*  pre-installed [pgBackRest](../platform/managing/pgbackrest.mdx) for simplified backup and restore capabilities.
...
After applying version-specific patches, you can configure, `make`, and `make install` the Postgres server as described in the [Postgres documentation](https://www.postgresql.org/docs/17/installation.html). When the build completes, add the location of your `pg_config` file to your `PATH` variable:
