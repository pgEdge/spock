...
| `events` | This view lists recent state changes of the node kept in shared memory: subscriptions enabled or disabled, workers started, stopped or failed, provider connects, extension upgrades. See [Monitoring a Node](node_monitoring.md). The view includes the following columns: `event_id`, `event_time`, `event_type`, `severity`, `subscription_id`, `subscription_name`, `pid`, `lsn`, `detail` |
...
| `node_status` | This view returns one row describing the whole node: versions, worker and subscription counts, WAL retained by slots, replication lag and the time of the last events. See [Monitoring a Node](node_monitoring.md). |
| `peer_status` | This view lists every node this node knows about with its interfaces and the subscription that brings its changes here. See [Monitoring a Node](node_monitoring.md). |
...
| `replication_set_status` | This view lists the replication sets of the local node with their replicate flags and the number of tables, sequences and subscriptions using each. |
...
| `settings` | This view shows the spock parameters and the core parameters replication depends on, with unit, source and pending restart. |
| `slot_status` | This view describes every spock replication slot of the database together with the walsender serving it, the WAL it holds back and the feedback lag. See [Monitoring a Node](node_monitoring.md). |
...
| `subscription_stats` | This view returns the activity counters of every subscription: worker starts and failures, provider connects, messages and transactions received, tuple and conflict counters, and the last error. See [Monitoring a Node](node_monitoring.md). |
| `subscription_status` | This view describes every subscription on one row: configuration, status, the apply worker serving it, replication positions and lag, and the last error. See [Monitoring a Node](node_monitoring.md). |
| `system_status` | This view returns one row about the host and the instance: disk space, load, connections, long and idle transactions, wraparound age, database statistics and WAL archiving. See [Monitoring a Node](node_monitoring.md). |
| `table_sync_status` | This view shows `local_sync_status` with readable `sync_kind` and `sync_status` values. |
...
| `worker_status` | This view lists the spock processes of the node: supervisor, manager, apply and sync workers, with their status and the subscription or table they serve. See [Monitoring a Node](node_monitoring.md). |
...
### Confirming that a Functioning Subscription Exists

You can connect to the database server and query the `spock.subscription`
table to return information about a subscription:
...
You can query the `spock.tables` table to identify tables that are not yet
part of a replication set; if the set name is `null`, the table is not part
