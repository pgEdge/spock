...
- [Arguments Client Supplies to Output Plugin](#arguments-client-supplies-to-output-plugin)
...
The message after the `BEGIN` may be a *forwarded transaction origin* message
...
is of arbitrary and application-defined format. Applications *should* prefix
...
The row's relidentifier *must* match that of the most recently preceding
metadata message. All consecutive row messages must currently have the same
relidentifier. (*Later extensions to add metadata caching will relax these
requirements for clients that advertise caching support; see the documentation
on metadata messages for more detail*).
...
Row messages *may not* arrive except during a transaction as delimited by `BEGIN`
...
read is determined based on the first field, *kind*.
...
Unrecognised field *kind* values are a protocol error for the downstream. The
...
or even same transaction. *This requirement will later be weakened by the
addition of client metadata caching, which will be advertised to the upstream
with an output plugin parameter.*
...
libpq protocol FATAL message that terminates the session. See
[Startup Errors](#startup-errors) below.
