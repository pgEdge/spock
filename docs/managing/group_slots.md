...
oldest safe WAL position for an entire Spock replication group. They let a node
retain the WAL that any active or relevant downstream member of the group might
still need, and release it only once every required member has confirmed durable
progress for the current membership generation.
