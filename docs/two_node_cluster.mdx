...
4. Use the [`spock.node_create`](../spock_ext/spock_functions/functions/spock_node_create.md) command to create the provider and subscriber nodes; on `n1`, use the command:
...
5. On `n1`, use the [`spock.repset_add_all_tables`](../spock_ext/spock_functions/functions/spock_repset_add_all_tables.md) command to add the tables in the `public` schema to the `default` replication set.  If you are working in another schema, customize this command as needed:

    `SELECT spock.repset_add_all_tables('default', ARRAY['public']);`

6. On n2, use the [`spock.sub_create`](../spock_ext/spock_functions/functions/spock_sub_create.md) command to create the subscription between n2 and n1; the name of the subscription is `sub_n2_n1`:
...
8. To ensure that modifications to your [DDL statements are automatically replicated](../spock_ext/managing/spock_autoddl.mdx), connect to each node with a Postgres client and invoke the following SQL commands:
...
9. Then, [check the status](../spock_ext/monitoring/spock_info.md) of each node:
