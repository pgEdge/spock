...
[Disable auto_ddl](../../platform/managing/autoddl.mdx) on all cluster nodes.
...
Prepare the new node to meet the prerequisites described [here](../../platform/prerequisites.mdx).
...
6. In the next step, we use a  [`spock.sync_event`](../../spock_ext/monitoring/spock_sync_event.mdx) to confirm transactions are caught up on our source node; we want to ensure that all of the transactions from `n3` are part of the `n1` data set:
...
7. Next, use a [`spock.sync_event`](../../spock_ext/monitoring/spock_sync_event.mdx) to confirm transactions from `n2` are caught up on our provider node: 
...
16. Finally, on `n4`, we can use a SQL command to verify that [replication lag](../../spock_ext/monitoring/lag_tracking.mdx) is at an acceptable level on all nodes:
