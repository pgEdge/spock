...
then
  ...
  psql -U $DBUSER -d $DBNAME -h /tmp -c "begin;" 
  psql -U $DBUSER -d $DBNAME -h /tmp -c "insert into t2 values(1,100);" 

  psql -U $DBUSER -d $DBNAME -h ${peer_names[0]} -c "begin;" 
  psql -U $DBUSER -d $DBNAME -h ${peer_names[0]} "insert into t2 values(1,200);" 

  psql -U $DBUSER -d $DBNAME -h /tmp -c "commit;" 
  psql -U $DBUSER -d $DBNAME -h ${peer_names[0]} -c "commit;"
