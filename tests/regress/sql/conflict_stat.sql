...
-- ============================================================
-- Test spock.get_subscription_stats() against a disabled subscription
-- whose apply worker has never run (PG18 stats hooks guard, read side).
-- The pgstat entry is created at CREATE SUBSCRIPTION time regardless of
-- "enabled", so this must return a well-defined all-zero row, not crash.
-- ============================================================

BEGIN;
SELECT 1 FROM spock.sub_create(
    subscription_name := 'test_subscription_disabled',
...
SELECT sub_id AS disabled_sub_id FROM spock.subscription
    WHERE sub_name = 'test_subscription_disabled' \gset

SELECT confl_update_missing,
  confl_insert_exists,confl_update_origin_differs,confl_update_exists,
  confl_delete_origin_differs,confl_delete_missing,confl_delete_exists,
  stats_reset
FROM spock.get_subscription_stats(:disabled_sub_id);

SELECT spock.sub_drop('test_subscription_disabled');
