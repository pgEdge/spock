...
--
-- Two of those rely on an UPDATE failing because its row is gone.  With
-- spock.missing_update_to_insert on (the default) the row would be rebuilt
-- and no exception raised, so this test turns it off.  The conversion itself
-- is covered by conflict_stat and TAP 040.
--
