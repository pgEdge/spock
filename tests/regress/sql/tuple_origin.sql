...
\c :subscriber_dsn
SELECT current_setting('server_version_num')::int >= 180000 AS has_conflict_stats \gset
...
-- Reset conflict stats (PG18+ only)
\if :has_conflict_stats
SELECT sub_id AS origin_test_sub_id FROM spock.subscription
  WHERE sub_name = 'test_subscription' \gset
SELECT spock.reset_subscription_stats(:origin_test_sub_id);
\endif
...
-- Verify DELETE_MISSING stat counter (PG18+ only)
\if :has_conflict_stats
SELECT confl_delete_missing
FROM spock.get_subscription_stats(:origin_test_sub_id);
\endif
...
-- Verify DELETE_EXISTS stat counter (PG18+ only)
\if :has_conflict_stats
SELECT confl_delete_exists
FROM spock.get_subscription_stats(:origin_test_sub_id);
\endif
...
-- Verify INSERT_EXISTS stat counter (PG18+ only)
\if :has_conflict_stats
