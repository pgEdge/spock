...
# 4. Add node n3 using ZODAN add_node procedure
# 5. Verify n3 is properly integrated
# 6. Load ZODREMOVE procedures on n3
# 7. Remove node n3 using ZODREMOVE remove_node procedure
...
my $add_node_cmd = "$pg_bin/psql -p $n3_port -d $dbname -c \"
    CALL spock.add_node(
        'n1',
        'host=$host dbname=$dbname port=$node_ports->[0] user=$db_user password=$db_password',
        'n3',
...
if ($add_node_result == 0) {
    pass('add_node procedure executed successfully');
} else {
    fail("add_node procedure failed with exit code: $add_node_result");
}
...
my $remove_node_cmd = "$pg_bin/psql -p $n3_port -d $dbname -c \"
...
if ($remove_node_result == 0) {
    pass('remove_node procedure executed successfully');
} else {
    fail("remove_node procedure failed with exit code: $remove_node_result");
}
...
print "✓ Added node n3 using ZODAN add_node\n";
print "✓ Verified n3 integration\n";
print "✓ Removed node n3 using ZODREMOVE remove_node\n";
