...
#
# Fast path (local dev): set V5_PG_INSTALL and V60_PG_INSTALL env vars to
...
#                 V60_PG_INSTALL=/usr/local/pgsql/pg18
my $V5_PG_INSTALL  = $ENV{V5_PG_INSTALL}  // "$TEMP_BASE/pg_v5";
my $V60_PG_INSTALL = $ENV{V60_PG_INSTALL} // "$TEMP_BASE/pg_v60";
...
diag("V60_PG_INSTALL: $V60_PG_INSTALL");
diag("PG_TAG        : $PG_TAG");
...
END {
    ...
    remove_tree("$TEMP_BASE/build_v60")    if -d "$TEMP_BASE/build_v60";
...
# ─────────────────────────────────────────────────────────────────────────────
diag("PHASE 2: Ensuring Spock 6.0.0 environment ($V60_BRANCH)");
ok(ensure_version_ready($V60_BRANCH, $V60_PG_INSTALL, 'v60'),
...
# ─────────────────────────────────────────────────────────────────────────────
diag("PHASE 4: Initialising fresh node (Spock 6.0.0)...");
ok(init_and_start_node($DATADIR_NEW, $PORT_NEW, $V60_PG_INSTALL),
   'Fresh node started (6.0.0)');
...
like($ver_new, qr/^6\.0\.0/, 'Fresh node installs at 6.0.0');

# ─────────────────────────────────────────────────────────────────────────────
# PHASE 6 – Upgrade node 1: swap to 6.0.0 postgres binary, ALTER EXTENSION UPDATE
# ─────────────────────────────────────────────────────────────────────────────
diag("PHASE 6: Stopping upgrade node to swap to Spock 6.0.0 binary...");
stop_pg($DATADIR_UPG);

# Restart with the 6.0.0 postgres binary.  shared_preload_libraries='spock'
# (bare name) resolves against the new binary's own $libdir — no conf change needed.
_start_postgres($V60_BIN, $DATADIR_UPG);
ok(wait_for_pg_ready($HOST, $PORT_UPG, $PG_BIN, 30),
   'Upgrade node restarted with 6.0.0 binary');
...
is($ver_upg2, '6.0.0', 'Upgrade node extension version is now 6.0.0');
