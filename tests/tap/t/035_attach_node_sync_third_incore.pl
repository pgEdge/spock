...
# Drain replication backlog before attach_node so apply workers are less busy
# during slot creation, increasing the chance of an idle inter-commit gap.
print STDERR "Draining replication before attach_node ...\n";
psql_or_bail(1, 'SELECT spock.wait_slot_confirm_lsn(NULL, NULL)');
psql_or_bail(2, 'SELECT spock.wait_slot_confirm_lsn(NULL, NULL)');

psql_or_bail(3,
	"CALL spock.attach_node(src_node_name := 'n1',
...
# Wait for replication to stabilize after attach_node before checking.
# pgbench is still running so lag won't reach zero; just drain current backlog.
print STDERR "Waiting for replication to settle after attach_node ...\n";
psql_or_bail(1, 'SELECT spock.wait_slot_confirm_lsn(NULL, NULL)');
psql_or_bail(2, 'SELECT spock.wait_slot_confirm_lsn(NULL, NULL)');
sleep(5);
...
psql_or_bail(3, "CALL spock.detach_node(
	target_node_name := 'n3',
...
# Let the cluster settle after detach_node before starting the next cycle.
print STDERR "Waiting for cluster to settle after detach_node ...\n";
...
# Drain replication backlog before second attach_node.
print STDERR "Draining replication before second attach_node ...\n";
psql_or_bail(1, 'SELECT spock.wait_slot_confirm_lsn(NULL, NULL)');
psql_or_bail(2, 'SELECT spock.wait_slot_confirm_lsn(NULL, NULL)');
...
# Wait for replication to stabilize after second attach_node.
# pgbench is still running so lag won't reach zero; just drain current backlog.
print STDERR "Waiting for replication to settle after attach_node ...\n";
psql_or_bail(1, 'SELECT spock.wait_slot_confirm_lsn(NULL, NULL)');
psql_or_bail(2, 'SELECT spock.wait_slot_confirm_lsn(NULL, NULL)');
...
scalar_query(2, "SELECT * FROM wait_subscription(remote_node_name := 'n3',
                                                  timeout := '3 minutes', delay := 0.5)");
scalar_query(3, "SELECT * FROM wait_subscription(remote_node_name := 'n1',
                                                  timeout := '3 minutes', delay := 0.5)");
scalar_query(3, "SELECT * FROM wait_subscription(remote_node_name := 'n2',
                                                  timeout := '3 minutes', delay := 0.5)");
