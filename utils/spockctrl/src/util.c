...
int
is_valid_json(const char *json_str)
{
	json_error_t error;
	json_t	   *json;
...
char *
get_json_string_value(json_t * json, const char *key)
{
...
json_t *
get_json_array(json_t * json, const char *key)
{
...
substitute_sql_vars(const char *sql_stmt)
{
	...
	while (*src && left > 1)
	{
		...
		if (nlen >= NODE_MAX || klen >= KEY_MAX)
			goto fail;
		memcpy(node, src + m[1].rm_so, nlen);
		node[nlen] = '\0';
		memcpy(key, src + m[2].rm_so, klen);
		key[klen] = '\0';
