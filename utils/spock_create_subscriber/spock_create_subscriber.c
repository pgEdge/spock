 ...
 * source's catalog before DROP EXTENSION spock removes it (Step 11a).
 * Utility-side memory only; never written to the manifest.
 */
 ...
 *		quotes -- caller wraps in "...").  Control characters below 0x20 are
 *		emitted as \uXXXX.  jsonapi.h provides a parser but no encoder,
 *		so this is a small local helper.
 */
static void
...
/*
 * check_spock_version_at_least_6
 *		Applied to the source and every peer: an old apply worker would
 *		advance the wrong-named origin, so version must be checked
 ...
 *		catalog, not just a count: exactly one healthy (status =
 *		'replicating') subscription from every other node in the set, no
 *		self-reference, no edge from outside the set, and no duplicate edge
 *		from the same origin regardless of status.
 */
static void
 ...
 *		under, so a node-name collision or wrong DSN can't silently
 *		validate the mesh against the wrong node.
 */
static void
...
/*
 * RepsetFingerprintEntry / compute_repset_fingerprints
 *		Canonical, comparable representation of one replication set as
 *		defined by its owning node: operation flags, each member table
 *		(sorted, with column list, row filter, and schema), then each
 *		member sequence (sorted).
 *
 *		spock.replication_set is keyed UNIQUE(set_nodeid, set_name) -- a
 *		set replicated via DDL becomes the replaying node's own row, not
 *		an echo -- so this is scoped by node_id to compare each node's
 *		own definition against another's.
 *
 *		selected_filter restricts this to sets actually referenced by a
 *		subscription's sub_replication_sets, since unused/scratch repsets
 *		can legitimately differ between nodes without affecting
 *		correctness.
 */
...
/*
 * build_selected_set_name_filter
 *		A SQL boolean expression ("set_name IN (...)") over the union of
 *		every replication set actually referenced by conn's own
 *		subscriptions (sub_replication_sets), rather than every
 *		replication set that happens to exist locally.  Caller frees the
 *		result.
 */
static char *
...
/*
 * check_replication_set_equivalence
 *		The forwarding path (peer -> source -> n3) and the future direct
 *		path (peer -> n3) must select exactly the same changes, or a
 *		change omitted on one path is lost once the direct subscription
 *		takes over.  Compare every selected replication set's fingerprint
 *		between the source and each peer; reject any mismatch or missing
 *		or extra set on either side.
 */
static void
 ...
 *		for a bidirectional join: Spock >= 6.0.0 on every node,
 *		track_commit_timestamp on, no pending DDL, an actual full-mesh
 *		subscription graph (not just a count), replication-set/schema
 *		equivalence across the source and every peer, and peer
 *		connectivity.
 */
static void
...
/*
 * check_single_spock_database
 *		The physical-backup path runs once per data directory, so it
 *		requires exactly one spock-configured database on the source
 *		instance.  Checked against actual spock configuration, not against
 *		--databases/--provider-dsn, since the instance can host other
 *		unrelated databases.
 *
 *		Fails closed: any database we cannot inspect aborts the run
 *		rather than being treated as spock-free.  datallowconn is not used
 *		to skip databases -- a database with connections disabled can
 *		still hold spock catalog state -- only true templates are
 *		excluded.
 */
static void
...
/*
 * check_no_native_subscriptions
 *		A physical base backup copies native (non-spock) logical
 *		subscriptions too, which DROP EXTENSION spock doesn't touch.  Once
 *		n3 is promoted and restarted, an enabled native subscription
 *		would start consuming from its provider as a second, unintended
 *		consumer.  pg_subscription is a shared catalog, so one query sees
 ...
 *		reverse subscriptions that were created during a previous join
 *		attempt.  Connectivity failures and drop errors are logged as
 *		warnings rather than being fatal, so cleanup keeps going and
 *		attempts every remaining resource -- but they are tracked: returns
 *		true only if every recorded resource was confirmed gone.  The
 *		manifest/sidecar record (the only way to retry) is removed *only*
 *		on a true return; an incomplete cleanup keeps it and the caller
 *		exits non-zero.
 */
static bool
...
main(int argc, char **argv)
{
	...
	for (dbnum = 0; dbnum < n_databases; dbnum++)
	{
		 ...
		 * "sub_<subscriber>_<source>" slot naming convention (Step 4).
		 * Manifest write is deferred until after the basebackup -- see the
		 * comment there.
		 */
		if (bidir.enabled)
		{
	...
	if (bidir.enabled)
	{
		...
		/* Step 11a: capture repset/table/sequence state before Step 11c. */
		source_nodeid = get_local_node_id(subscriber_conn);
		capture_catalog_state(subscriber_conn, source_nodeid, &capture);

		/* Steps 11b-11c: drop all origins, then guarded DROP EXTENSION. */
	...
	if (bidir.enabled)
	{
		...
		/* Step 15b: restore what Step 11a captured before the catalog strip. */
...
/*
 * get_local_node_id
 *		The connected node's own local node id, from spock.local_node --
 *		a plain catalog table, so it works even with spock's shared
 *		memory not loaded (e.g. before DROP EXTENSION, while spock is
 *		disabled).
 */
static Oid
 ...
 *		Capture replication-set definitions, table memberships, and
 *		sequence state from the local catalog before DROP EXTENSION
 *		removes it -- this reflects what was replicated at backup time,
 *		unlike querying the live source afterward.  Utility-side memory
...
capture_catalog_state(PGconn *conn, Oid source_nodeid, CatalogCapture *capture)
{
	...
	/* Replication set definitions owned by the source node. */
	printfPQExpBuffer(query,
					  "SELECT set_name, replicate_insert, replicate_update,"
					  " replicate_delete, replicate_truncate"
	...
	/* Table memberships across all of the source's sets. */
	printfPQExpBuffer(query,
	...
	/*
	 * Sequences and the sets they belong to (a sequence can be in more
	 * than one set, so this is captured per-membership like table rows,
	 * not deduplicated by sequence).
	 */
	printfPQExpBuffer(query,
...
/*
 * remove_unwanted_data_bidir
 *		Bidirectional-mode catalog strip (Steps 11b-11c): drop ALL
 *		replication origins (not just ones with a status row, unlike
 *		remove_unwanted_data()), then guard DROP EXTENSION ... CASCADE with
 *		a one-hop pg_depend inventory -- any non-spock object depending on
 *		a spock member would otherwise be silently collaterally dropped.
 */
static void
remove_unwanted_data_bidir(PGconn *conn, CatalogCapture *capture)
{
	...
	/*
	 * Step 11b: drop all replication origins copied by the basebackup.
	...
	/* Step 11c: guard against CASCADE collaterally dropping user objects. */
	res = PQexec(conn,
				 "WITH spock_ext AS ("
...
restore_replication_sets(PGconn *conn, CatalogCapture *capture)
{
	...
	/*
	 * 2.  Restore table memberships for all sets.  Named arguments are
	 * required: repset_add_table's 3rd positional argument is
	 * synchronize_data, not the column list, so a positional call would
	 * misfire.
	 *
	 * include_partitions := false: the capture already has a separate row
	 * per partition.  Restoring the parent with include_partitions := true
	 * would re-add every child, violating the (set_id, set_reloid)
	 * primary key against the child's own captured row.
	 */
	for (i = 0; i < capture->num_tables; i++)
	{
	...
	/*
	 * 3.  Restore each sequence's replication-set membership, then its
	 * value, so n3 both publishes it and resumes it exactly.
	 */
	for (i = 0; i < capture->num_sequences; i++)
	{
...
/*
 * verify_replication_sets_restored
 *		Round-trip check: compare what actually landed against the
 *		capture, rather than trusting that each individual
 *		repset_add_table()/repset_add_seq() call succeeding means the
 *		final state matches.  Catches per-row drift (a column list or
 *		row_filter that didn't re-parse identically) and aggregate drift
 *		(an accidental double-add).  n3 is brand-new here, so an
 *		unqualified COUNT(*) is safe.
 */
static void
verify_replication_sets_restored(PGconn *conn, CatalogCapture *capture)
{
	 ...
	 * restore_replication_sets()'s argument binding would otherwise pass
	 * verification with wrong flags on the clone.
	 */
	for (i = 0; i < capture->num_repsets; i++)
	{
		RepsetCapture *s = &capture->repsets[i];
...
/*
 * reset_subscriber_sysid
 *		Assign data_dir a fresh system identifier, since a physical clone
 *		otherwise keeps the source's -- risking stray WAL from one
 *		cluster being mistaken for the other's, and leaving
 *		system_identifier useless for proving --subscriber-dsn actually
 *		reaches this node.  Called with the subscriber stopped, right
 *		after promotion and before any catalog mutation.
 *
 *		pg_resetwal alone does NOT do this: it only regenerates
 *		system_identifier when it can't read an existing control file at
 *		all (verified empirically -- running it against a valid,
 *		cleanly-shut-down cluster leaves the identifier untouched).  The
 *		identifier is overwritten directly in the control file here;
 *		pg_resetwal is then run afterward (run_pg_resetwal()) only to
 *		relabel the existing WAL segments to match.
 *
 *		Returns the new identifier as a string (caller must free()),
 *		matching read_sysid()'s convention.
 */
static char *
 ...
 *		identifier reset_subscriber_sysid() just wrote to the control
 *		file (see that function's comment for why both steps are
 *		needed).  Must run after reset_subscriber_sysid(), with the
 *		subscriber stopped.
 */
static void
